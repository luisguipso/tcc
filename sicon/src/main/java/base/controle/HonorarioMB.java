	...
	public void gerarHonorario() {
		...
		if (!listaHonorario.isEmpty()) {
			if (!listaClientes.isEmpty()) {
				for (Cliente cli : listaClientes) { //para cada cliente ter honor�rio ser� falso
					...
					for (Honorario hon : listaHonorario) {
						...
						if (cli.getId() == hon.getCliente().getId()) {
							clienteTemHonorario = true;                //para cada cliente cadastrado em honor�rio, a variavel ser� true
						}
					}
					// pra cada cliente que nao tiver honor�rio para a competencia passada na tela, sera criado um honor�rio com valores padr�o
		...
		// se lista honorarios vazia, ou seja, nao ha nenhum honorario para a competencia passada
			...
			System.out.println("criando honorarios para todos os clientes"); //para cada cliente um honor�rio ser� criado
